...
1. Create `src/themes/presets/your-theme.ts` implementing the `Theme` interface
2. Add your theme name to the `ThemeName` union in `src/themes/types.ts`
3. Register it in `src/themes/index.ts`

## Adding a Template

1. Create `src/templates/your-template.ts` implementing the `Template` interface
2. Register it in `src/templates/index.ts`

See the Korean section above for detailed code examples — the interfaces and file structure are the same.
